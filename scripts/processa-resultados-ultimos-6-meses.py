...
all_files.extend(glob.glob(path + f"/download/*{y-1}.csv"))
print(all_files)
...
# delimitando faicha de datas de atendimentos dos últimos 6 meses
dmin = df['Data de abertura'].max() - dateutil.relativedelta.relativedelta(months=6)
df = df.loc[df['Data de abertura'] >= dmin]
...
if not os.path.exists(f'resultados/ultimos-6-meses'):
    os.makedirs(f'resultados/ultimos-6-meses')

for t in poligonos.Tema.unique():
    if not os.path.exists(f'resultados/ultimos-6-meses/{slugify(t)}/poligonos'):
        os.makedirs(f'resultados/ultimos-6-meses/{slugify(t)}/poligonos')
    ...
    for s in poligonos[(poligonos.Tema == t)].Serviço.unique():
        ...
        if len(r) > 1:
            ...
            r.to_file(f'resultados/ultimos-6-meses/{slugify(t)}/poligonos/{slugify(s)}.gpkg', driver='GPKG')
            regs.append([y, t, s, 'poligono', f'resultados/ultimos-6-meses/{slugify(t)}/poligonos/{slugify(s)}.gpkg', len(r)])
...
for t in pontos.Tema.unique():
    if not os.path.exists(f'resultados/ultimos-6-meses/{slugify(t)}/pontos'):
        os.makedirs(f'resultados/ultimos-6-meses/{slugify(t)}/pontos')
    ...
    for s in pontos[(pontos.Tema == t)].Serviço.unique():
        ...
        if len(r) > 1:
            r.to_file(f'resultados/ultimos-6-meses/{slugify(t)}/pontos/{slugify(s)}.gpkg', driver='GPKG')
            regs.append([y, t, s, 'ponto', f'resultados/ultimos-6-meses/{slugify(t)}/poligonos/{slugify(s)}.gpkg', len(r)])


# Abrindo o CSV de resultados
file = open('resultados/resultados-ultimos-6-meses.csv', 'w+', newline ='') 
